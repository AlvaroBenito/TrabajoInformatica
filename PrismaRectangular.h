...
protected:
	Vector3D p1;//Vertice1
	Vector3D p2;//Vertice2
	static int tipoMaterial;
	bool impacto;//Variable que usan las clases hijas de "PrismaRectangular" para que solo se aplique una colision con el personaje
public:
	PrismaRectangular(Vector3D coor1, Vector3D coor2);
	PrismaRectangular();
	void dibuja();
	static void dibuja2(Vector3D a, Vector3D b);//Funcion para dibujar un plano sin necesidad de objeto
	Vector3D getP1();
	Vector3D getP2();
