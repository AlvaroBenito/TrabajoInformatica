...
void Mapa::dibuja() {         //funcion para dibujar las 18 plataformas(18 porque se deja de ver la construccion al final de las plataformas)
	if (inicializa == false) {//Hay que primero dibujar 18 para iniciar, y luego la funcion cambia se encargará de modificar sus posiciones
...
void Mapa::cambia(float mult) {//funcion para destruir y volver a crear las plataformas:
							   ...
							   //Si el suelo se moviera con el personaje, parecería que está quieto
	static float cuenta = 25;
	if (cuenta < 0) {
		camino->setZ(-10 * (float)(++n));
		delete[]camino;//Se destruyen las plataformas para luego volver a crearlas en otra posición
		camino = new Plataforma[18];
		cuenta += 25/mult;
	}
	cuenta-=1;
